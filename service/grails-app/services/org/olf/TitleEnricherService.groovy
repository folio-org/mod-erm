        ...
        if (cache_updater.requiresSecondaryEnrichmentCall()) {
          ...
          if (ti) {
            ...
            if (!enrichedIdSet.contains(ti.id)) {
              ...
              Map titleInstanceEnrichmentValues = [:]
              titleInstanceEnrichmentValues = cache_updater.getTitleInstance(kb.name, kb.uri, sourceIdentifier, ti?.type?.value, ti?.subType?.value)
